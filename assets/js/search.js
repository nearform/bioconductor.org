...
var getSearchUrl = function (query, start) {
  var url =
    "//master.bioconductor.org/solr/default/select?indent=on&version=2.2&q=" +
    query +
    // boost release and devel landing page paths, and fuzzy title matches
    " id:*\\/release\\/bioc\\/html*^1.2 id:*\\/devel\\/bioc\\/html*^1.1 title:" +
    query +
    "~" +
    "&fq=&start=" +
    start +
...
var searchResponse = function (data) {
  ...
  if (numFound == 0) $("#search_results").html("");
  ...
  if (numFound > 0) {
    ...
    for (var i = 0; i < docs.length; i++) {
      ...
      stringToAppend +=
        "href='" + doc.id + "'>" + title + "</a> - " + doc.id + "</dt><dd>";
      if (isR) {
        stringToAppend += "<pre>";
      }
    ...
    if (start >= rows) {
      ...
      jQuery(".previous_search_page").html(
...
async function getSearchRes(url) {
  try {
    const res = await fetch(url);
    const resJson = await res.json();
    searchResponse(resJson);
  } catch (e) {
    ajaxErrHandler();
    throw e;
  }
